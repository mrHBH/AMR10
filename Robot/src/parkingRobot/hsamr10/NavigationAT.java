...
import parkingRobot.hsamr10.Navigation.NavigationHandler;
...
public class NavigationAT implements INavigation{
	...
	/**
	 * Navigation handler containing map and information about current position
	 */
...
//		monitor.writeNavigationComment("Navigation");
	...
	public synchronized Pose getPose(){
		return navigationHandler.getPose();
	}
	/* (non-Javadoc)
	...
	private void calculateLocation(){
		...
		if (R.isNaN()) { //robot don't move
			xResult			= this.navigationHandler.getMeasurementPose().getX();
			yResult			= this.navigationHandler.getMeasurementPose().getY();
			angleResult 	= this.navigationHandler.getMeasurementPose().getHeading();
		} else if (R.isInfinite()) { //robot moves straight forward/backward, vLeft==vRight
			xResult			= this.navigationHandler.getMeasurementPose().getX() + vLeft * Math.cos(this.navigationHandler.getMeasurementPose().getHeading()) * deltaT;
			yResult			= this.navigationHandler.getMeasurementPose().getY() + vLeft * Math.sin(this.navigationHandler.getMeasurementPose().getHeading()) * deltaT;
			angleResult 	= this.navigationHandler.getMeasurementPose().getHeading();
		} else {			
			ICCx = this.navigationHandler.getMeasurementPose().getX() - R.doubleValue() * Math.sin(this.navigationHandler.getMeasurementPose().getHeading());
			ICCy = this.navigationHandler.getMeasurementPose().getY() + R.doubleValue() * Math.cos(this.navigationHandler.getMeasurementPose().getHeading());
		
			xResult 		= Math.cos(w * deltaT) * (this.navigationHandler.getMeasurementPose().getX()-ICCx) - Math.sin(w * deltaT) * (this.navigationHandler.getMeasurementPose().getY() - ICCy) + ICCx;
			yResult 		= Math.sin(w * deltaT) * (this.navigationHandler.getMeasurementPose().getX()-ICCx) + Math.cos(w * deltaT) * (this.navigationHandler.getMeasurementPose().getY() - ICCy) + ICCy;
			angleResult 	= this.navigationHandler.getMeasurementPose().getHeading() + w * deltaT;
		}
		
		this.navigationHandler.getMeasurementPose().setLocation((float)xResult, (float)yResult);
		this.navigationHandler.getMeasurementPose().setHeading((float)angleResult);		 
	}
